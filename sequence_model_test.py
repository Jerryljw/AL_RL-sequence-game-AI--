...
class SequenceGameRule(GameRule):
    ...
    def gameEnds(self): #Game ends if a team has formed at least 2 sequences, or if the deck is empty.
        ...
        # NEW ADD for calculating the last action to update q value
        if scores[RED]>=2 or scores[BLU]>=2 or len(self.current_game_state.board.draft)==0:

            count = 1
            play_weight = AdvancedDict()
            remove_weight = AdvancedDict()
            draft_weight = AdvancedDict()
            for line in open("QlearnWeight.txt", "r"):
                line = line.strip()
                if count == 1:
                    draft_weight = eval(line)
                    count += 1
                elif count == 2:
                    remove_weight =eval(line)
                    count += 1
                else:
                    play_weight =eval(line)
            ...
            type = ""
            preValue = 0
            for line in open("QlearnFeature.txt","r"):
                line = line.strip()
                if count == 1:
                    type = line.strip()
                    count += 1
                    ...
                    preValue = float(values[0])
                    if type == "p":
                        print(values[1])
                        play_feature = eval(values[1])
                    else:
                        remove_feature = eval(values[1])
            ...
            if type == "p":
                for key in play_feature.keys():
                    ...
                    play_weight[key] += ALPHA * (reward - preValue) * play_feature[key]
            ...
            if type == "remove":
                for key in remove_feature.keys():
                    ...
                    remove_weight[key] += ALPHA * (reward - preValue) * remove_feature[key]
                f = open("QlearnFeature.txt", 'w')
                f.write("r\n")
                f.write(str(remove_feature))

            f = open("QlearnWeight.txt", 'w')
            f.write(str(draft_weight))
            f.write("\n")
            f.write(str(remove_weight))
            f.write("\n")
            f.write(str(play_weight))
            f.write("\n")
